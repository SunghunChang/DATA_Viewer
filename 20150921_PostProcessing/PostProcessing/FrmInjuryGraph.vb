...
Public Class FrmInjuryGraph
    ...
    Dim GraphBasedCorrelation As Boolean '그래프 기반 Correlation을 했는지 저장하는 변수

    Dim DummyType() As Integer
    ...
    Private Sub FrmInjuryGraph_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        If e.KeyCode = Keys.F5 Then
            Me.ReloadAnalysisDATAToolStripMenuItem_Click(e, Nothing)
        End If
    End Sub
    ...
    Private Sub FrmInjuryGraph_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        With Me
            ...
            .GraphTab.TabPages(0).BackColor = Color.White
            .GraphTab.TabPages(1).BackColor = Color.White
            .GraphTab.TabPages(2).BackColor = Color.White
        ...
        For i = 0 To UBound(GraphTitle) - 1
            Me.ListBox1.Items.Add(GraphTitle(i))
        Next
        For i = 0 To 7
            Me.ListBox1.Items.Add(GraphTitle(UBound(GraphTitle)) & " #" & i + 1)
        Next
        EventPass = False
    ...
    Public Sub New(ByVal TFilePath() As String, ByVal FileNames() As String, ByVal para_arr1(,,) As Integer, ByVal para_arr2(,,) As Integer, ByVal C_Factor() As Decimal, ByVal IsGraphBased As Boolean, ByVal D_Type() As Integer)
        ...
        GraphBasedCorrelation = IsGraphBased
        ...
        Dim NeckGraphNum(8) As Integer
        'Graph Based Related

        If IsGraphBased = True Then

            'Nij/Mocy 그래프 번호를 찾는다.
            Dim InjuryFile As StreamReader
            InjuryFile = New StreamReader(Application.StartupPath & "\Profile\GraphSetting.Gcorrel")
            Dim Tmp_Str() As String
            Tmp_Str = InjuryFile.ReadLine.Split("\") 'Head Res. Acc.
            NeckGraphNum(0) = CInt(Tmp_Str(1))
            Tmp_Str = InjuryFile.ReadLine.Split("\") 'Fx
            NeckGraphNum(1) = CInt(Tmp_Str(1))
            Tmp_Str = InjuryFile.ReadLine.Split("\") 'Fz
            NeckGraphNum(2) = CInt(Tmp_Str(1))
            Tmp_Str = InjuryFile.ReadLine.Split("\") 'My
            NeckGraphNum(3) = CInt(Tmp_Str(1))
            Tmp_Str = InjuryFile.ReadLine.Split("\") 'Mocy
            NeckGraphNum(4) = CInt(Tmp_Str(1))
            Tmp_Str = InjuryFile.ReadLine.Split("\") 'NTE
            NeckGraphNum(5) = CInt(Tmp_Str(1))
            Tmp_Str = InjuryFile.ReadLine.Split("\") 'NTF
            NeckGraphNum(6) = CInt(Tmp_Str(1))
            Tmp_Str = InjuryFile.ReadLine.Split("\") 'NCE
            NeckGraphNum(7) = CInt(Tmp_Str(1))
            Tmp_Str = InjuryFile.ReadLine.Split("\") 'NCF
            NeckGraphNum(8) = CInt(Tmp_Str(1))

            InjuryFile.Close()

            '목상해 데이터를 만들고 읽어온다.
            'DummyType() as integer
        ...
        For k = 0 To UBound(Names)
            ...
            Dim Neck_Mocy() As Double = Nothing
            Dim NTE() As Double = Nothing
            Dim NTF() As Double = Nothing
            Dim NCE() As Double = Nothing
            Dim NCF() As Double = Nothing

            If IsGraphBased = True Then
                'Graph Based Injury일 경우 목상해 그래프를 다시 그린다. (데이터를 새로 읽어옴)
                'DummyType() as integer
                '0=50% H-3 / 1=5% H-3 / 2=50% THOR
               
                'Neck Fx Reading
                Analopen.DataReading(Path(k), Names(k), Graph_Ext(NeckGraphNum(1)), DATA_Pos(NeckGraphNum(1), k, 0), DATA_Pos(NeckGraphNum(1), k, 1), DATA_Tot_Len(NeckGraphNum(1), k, 0), DATA_Tot_Len(NeckGraphNum(1), k, 1), TimeVals, DataVals, C_Factor(NeckGraphNum(1)))
                Neck_Fx = DataVals
                'Neck Fz Reading
                Analopen.DataReading(Path(k), Names(k), Graph_Ext(NeckGraphNum(2)), DATA_Pos(NeckGraphNum(2), k, 0), DATA_Pos(NeckGraphNum(2), k, 1), DATA_Tot_Len(NeckGraphNum(2), k, 0), DATA_Tot_Len(NeckGraphNum(2), k, 1), TimeVals, DataVals, C_Factor(NeckGraphNum(2)))
                Neck_Fz = DataVals
                'Neck My Reading
                Analopen.DataReading(Path(k), Names(k), Graph_Ext(NeckGraphNum(3)), DATA_Pos(NeckGraphNum(3), k, 0), DATA_Pos(NeckGraphNum(3), k, 1), DATA_Tot_Len(NeckGraphNum(3), k, 0), DATA_Tot_Len(NeckGraphNum(3), k, 1), TimeVals, DataVals, C_Factor(NeckGraphNum(3)))
                Neck_My = DataVals
                Neck_Mocy = CalMocyDATA(Neck_Fx, Neck_My, DummyType(k))
                NTE = CalNTEDATA(Neck_Fz, Neck_Mocy, DummyType(k))
                NTF = CalNTFDATA(Neck_Fz, Neck_Mocy, DummyType(k))
                NCE = CalNCEDATA(Neck_Fz, Neck_Mocy, DummyType(k))
                NCF = CalNCFDATA(Neck_Fz, Neck_Mocy, DummyType(k))
            End If
            ...
            For i = 0 To 31
                ...
                '데이터를 읽어오는 함수
                If IsGraphBased = True And (DummyType(k) <> 2) Then

                    Select Case i

                        Case NeckGraphNum(4) 'Mocy
                            '데이터를 그래프에 올리는 함수
                            AddSeries(GraphCollection.Item(i + 1), TimeVals, Neck_Mocy, k + 1)
                        Case NeckGraphNum(5) 'NTE
                            AddSeries(GraphCollection.Item(i + 1), TimeVals, NTE, k + 1)
                        Case NeckGraphNum(6) 'NTF
                            AddSeries(GraphCollection.Item(i + 1), TimeVals, NTF, k + 1)
                        Case NeckGraphNum(7) 'NCE
                            AddSeries(GraphCollection.Item(i + 1), TimeVals, NCE, k + 1)
                        Case NeckGraphNum(8) 'NCF
                            AddSeries(GraphCollection.Item(i + 1), TimeVals, NCF, k + 1)
                        Case Else
                            Analopen.DataReading(Path(k), Names(k), Graph_Ext(i), DATA_Pos(i, k, 0), DATA_Pos(i, k, 1), DATA_Tot_Len(i, k, 0), DATA_Tot_Len(i, k, 1), TimeVals, DataVals, C_Factor(i))
                            '데이터를 그래프에 올리는 함수
    ...
    Private Sub ReloadSeries(ByRef Charts As Windows.Forms.DataVisualization.Charting.Chart, ByRef Xaxis() As Decimal, ByRef Yaxis() As Double, ByVal CaseNum As Integer)
        With Charts
            .Series(CaseNum - 1).Points.Clear()
            .Series(CaseNum - 1).Points.DataBindXY(Xaxis, Yaxis)
            .Series(CaseNum - 1).ChartType = DataVisualization.Charting.SeriesChartType.Line
        End With
    End Sub
    ...
    Private Sub FrmInjuryGraph_Resize(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Resize
        ...
        If Me.Size.Width > 100 And Me.Size.Height > 100 Then
            With Me
                ...
                .Chart32.Location = New Point(.AnalBox.Location.X + .AnalBox.Width + 2, .AnalBox.Location.Y)
                .Chart32.Size = New Size(.AnalBox.Width, .AnalBox.Height / 4)
                .Chart33.Location = New Point(.Chart32.Location.X + .Chart32.Width, .AnalBox.Location.Y)
                .Chart33.Size = New Size(.Chart32.Width, .Chart32.Height)
                .TESTBox.Location = New Point(.Chart33.Location.X + .Chart33.Width + 2, .AnalBox.Location.Y)
                .TESTBox.Size = New Size(.AnalBox.Width + 1, .AnalBox.Height)
                .Chart34.Location = New Point(.Chart32.Location.X, .Chart33.Location.Y + .Chart33.Height)
                .Chart34.Size = New Size(.Chart32.Width, .Chart32.Height)
                .Chart35.Location = New Point(.Chart34.Location.X + .Chart34.Width, .Chart34.Location.Y)
                .Chart35.Size = New Size(.Chart32.Width, .Chart32.Height)
                .Chart36.Location = New Point(.Chart34.Location.X, .Chart34.Location.Y + .Chart34.Height)
                .Chart36.Size = New Size(.Chart32.Width, .Chart32.Height)
                .Chart37.Location = New Point(.Chart36.Location.X + .Chart36.Width, .Chart36.Location.Y)
                .Chart37.Size = New Size(.Chart32.Width, .Chart32.Height)
                .Chart38.Location = New Point(.Chart36.Location.X, .Chart36.Location.Y + .Chart36.Height)
                .Chart38.Size = New Size(.Chart32.Width, .Chart32.Height)
                .Chart39.Location = New Point(.Chart38.Location.X + .Chart38.Width, .Chart38.Location.Y)
                .Chart39.Size = New Size(.Chart32.Width, .Chart32.Height)
    ...
    Private Sub TESToverlap(ByVal Dummy As Integer, ByVal DATAFileName As String, Optional ByVal DATAFilePath As String = "")
        ...
        For i = 0 To 31  '그래프 개수
            ...
            If TEST_Para_Num(i, 0) <> -1 And TEST_Para_Num(i, 1) <> -1 Then
                ...
                If Mid(DATAFileName, Len(DATAFileName) - 2, 4) = "tdm" Then
                    DrawingTESTgraphs(GraphCollection.Item(i + 1), Tmp_X_Vals, Tmp_Y_Vals, Tmp_CH_Group_Name_Sel(TDM_Sel_CHGroup_Num) & " " & DummyDisplayLegends, GraphTitle(i))
                Else
                    DrawingTESTgraphs(GraphCollection.Item(i + 1), Tmp_X_Vals, Tmp_Y_Vals, DATAFileName & DummyDisplayLegends, GraphTitle(i))
                End If
            Else
                '데이터가 없는 것들 그리는 함수
                Dim Zero_Arr(0) As Double
                If Mid(DATAFileName, Len(DATAFileName) - 2, 4) = "tdm" Then
                    DrawingTESTgraphs(GraphCollection.Item(i + 1), Zero_Arr, Zero_Arr, Tmp_CH_Group_Name_Sel(TDM_Sel_CHGroup_Num) & " " & DummyDisplayLegends, GraphTitle(i))
                Else
                    DrawingTESTgraphs(GraphCollection.Item(i + 1), Zero_Arr, Zero_Arr, DATAFileName & DummyDisplayLegends, GraphTitle(i))
                End If
    ...
    Private Sub Chart32_DoubleClick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Chart32.DoubleClick
        Dim PopUpForm As New InjuryPopUp(32, Me.Chart32, 0) ', Me.Chart00.Titles(0).Text)
        PopUpForm.Owner = Me
        PopUpForm.Show()
    End Sub
    Private Sub Chart33_DoubleClick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Chart33.DoubleClick
        Dim PopUpForm As New InjuryPopUp(32, Me.Chart33, 0) ', Me.Chart00.Titles(0).Text)
        PopUpForm.Owner = Me
        PopUpForm.Show()
    End Sub
    Private Sub Chart34_DoubleClick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Chart34.DoubleClick
        Dim PopUpForm As New InjuryPopUp(32, Me.Chart34, 0) ', Me.Chart00.Titles(0).Text)
        PopUpForm.Owner = Me
        PopUpForm.Show()
    End Sub
    Private Sub Chart35_DoubleClick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Chart35.DoubleClick
        Dim PopUpForm As New InjuryPopUp(32, Me.Chart35, 0) ', Me.Chart00.Titles(0).Text)
        PopUpForm.Owner = Me
        PopUpForm.Show()
    End Sub
    Private Sub Chart36_DoubleClick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Chart36.DoubleClick
        Dim PopUpForm As New InjuryPopUp(32, Me.Chart36, 0) ', Me.Chart00.Titles(0).Text)
        PopUpForm.Owner = Me
        PopUpForm.Show()
    End Sub
    Private Sub Chart37_DoubleClick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Chart37.DoubleClick
        Dim PopUpForm As New InjuryPopUp(32, Me.Chart37, 0) ', Me.Chart00.Titles(0).Text)
        PopUpForm.Owner = Me
        PopUpForm.Show()
    End Sub
    Private Sub Chart38_DoubleClick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Chart38.DoubleClick
        Dim PopUpForm As New InjuryPopUp(32, Me.Chart38, 0) ', Me.Chart00.Titles(0).Text)
        PopUpForm.Owner = Me
        PopUpForm.Show()
    End Sub
    Private Sub Chart39_DoubleClick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Chart39.DoubleClick
        Dim PopUpForm As New InjuryPopUp(32, Me.Chart39, 0) ', Me.Chart00.Titles(0).Text)
        PopUpForm.Owner = Me
        PopUpForm.Show()
    End Sub

    Private Sub Chart32_DragEnter(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DragEventArgs) Handles Chart32.DragEnter
        '두번째 : 효과
        e.Effect = DragDropEffects.Copy
    End Sub
    Private Sub Chart32_DragDrop(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DragEventArgs) Handles Chart32.DragDrop
        DragDropEventEnd(Chart32)
    End Sub
    Private Sub Chart33_DragEnter(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DragEventArgs) Handles Chart33.DragEnter
        '두번째 : 효과
        e.Effect = DragDropEffects.Copy
    End Sub
    Private Sub Chart33_DragDrop(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DragEventArgs) Handles Chart33.DragDrop
        DragDropEventEnd(Chart33)
    End Sub
    Private Sub Chart34_DragEnter(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DragEventArgs) Handles Chart34.DragEnter
        '두번째 : 효과
        e.Effect = DragDropEffects.Copy
    End Sub
    Private Sub Chart34_DragDrop(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DragEventArgs) Handles Chart34.DragDrop
        DragDropEventEnd(Chart34)
    End Sub
    Private Sub Chart35_DragEnter(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DragEventArgs) Handles Chart35.DragEnter
        '두번째 : 효과
        e.Effect = DragDropEffects.Copy
    End Sub
    Private Sub Chart35_DragDrop(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DragEventArgs) Handles Chart35.DragDrop
        DragDropEventEnd(Chart35)
    End Sub
    Private Sub Chart36_DragEnter(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DragEventArgs) Handles Chart36.DragEnter
        '두번째 : 효과
        e.Effect = DragDropEffects.Copy
    End Sub
    Private Sub Chart36_DragDrop(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DragEventArgs) Handles Chart36.DragDrop
        DragDropEventEnd(Chart36)
    End Sub
    Private Sub Chart37_DragEnter(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DragEventArgs) Handles Chart37.DragEnter
        '두번째 : 효과
        e.Effect = DragDropEffects.Copy
    End Sub
    Private Sub Chart37_DragDrop(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DragEventArgs) Handles Chart37.DragDrop
        DragDropEventEnd(Chart37)
    End Sub
    Private Sub Chart38_DragEnter(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DragEventArgs) Handles Chart38.DragEnter
        '두번째 : 효과
        e.Effect = DragDropEffects.Copy
    End Sub
    Private Sub Chart38_DragDrop(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DragEventArgs) Handles Chart38.DragDrop
        DragDropEventEnd(Chart38)
    End Sub
    Private Sub Chart39_DragEnter(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DragEventArgs) Handles Chart39.DragEnter
        '두번째 : 효과
        e.Effect = DragDropEffects.Copy
    End Sub
    Private Sub Chart39_DragDrop(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DragEventArgs) Handles Chart39.DragDrop
    ...
    Private Sub Chart32_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Chart32.KeyDown
        ...
        End If
    End Sub
    Private Sub chart33_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Chart33.KeyDown
        If e.KeyCode = Keys.Delete Then
            Dim i As Integer
        ...
        End If
    End Sub
    Private Sub chart34_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Chart34.KeyDown
        If e.KeyCode = Keys.Delete Then
            Dim i As Integer
        ...
        End If
    End Sub
    Private Sub chart35_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Chart35.KeyDown
        If e.KeyCode = Keys.Delete Then
            Dim i As Integer
        ...
        End If
    End Sub
    Private Sub chart36_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Chart36.KeyDown
        If e.KeyCode = Keys.Delete Then
            Dim i As Integer
        ...
        End If
    End Sub
    Private Sub chart37_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Chart37.KeyDown
        If e.KeyCode = Keys.Delete Then
            Dim i As Integer
        ...
        End If
    End Sub
    Private Sub chart38_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Chart38.KeyDown
        If e.KeyCode = Keys.Delete Then
            Dim i As Integer
        ...
        End If
    End Sub
    Private Sub chart39_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Chart39.KeyDown
        If e.KeyCode = Keys.Delete Then
            Dim i As Integer
    ...
    Private Sub ListBox1_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ListBox1.SelectedIndexChanged
        ...
        If Me.ListBox1.SelectedIndex < 32 Then
            ChartNameToChkLstBox(GraphCollection(Me.ListBox1.SelectedIndex + 1))
        Else
            ChartNameToChkLstBox(AddGraphCollection(Me.ListBox1.SelectedIndex + 1 - 32))
        End If
    ...
    Private Sub SeriesLstBox_ItemCheck(ByVal sender As Object, ByVal e As System.Windows.Forms.ItemCheckEventArgs) Handles SeriesLstBox.ItemCheck
        ...
        Else
            If Me.ListBox1.SelectedIndex > 31 Then
                'Additional Graph
                SeriesHideAndShow(AddGraphCollection(Me.ListBox1.SelectedIndex + 1 - 32))
            Else
                'Default Graph
    ...
    Private Sub SeriesLstBox_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SeriesLstBox.SelectedIndexChanged
        ...
        Else
            ChartHandlingMain_Start(AddGraphCollection(Me.ListBox1.SelectedIndex + 1 - 32))
        End If
    ...
    Private Sub BtnResetMain_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnResetMain.Click
        ...
        For k = 0 To UBound(Names)
            If Me.ListBox1.SelectedIndex > -1 And Me.ListBox1.SelectedIndex <= 31 Then
                '처음과 같이 그래프를 그린다.
                '데이터를 읽어오는 함수
                Analopen.DataReading(Path(k), Names(k), Graph_Ext(i), DATA_Pos(i, k, 0), DATA_Pos(i, k, 1), DATA_Tot_Len(i, k, 0), DATA_Tot_Len(i, k, 1), TimeVals, DataVals, ConversionF(i))
                '데이터를 그래프에 올리는 함수
                AddSeries(GraphCollection.Item(i + 1), TimeVals, DataVals, k + 1)
                'Display 정리
                DATAGraphDP.GraphAxisLine(GraphCollection(i + 1))
                Application.DoEvents()
            ElseIf Me.ListBox1.SelectedIndex >= 32 Then
                '그래프를 지운다.
                DelAllSeries(GraphCollection.Item(Me.ListBox1.SelectedIndex + 1))
            End If
        Next
        ...
        With Me
            If IsNumeric(.XscaleTxt.Text) And IsNumeric(.YscaleTxt.Text) And IsNumeric(.XoffsetTxt.Text) And IsNumeric(.YoffsetTxt.Text) Then
                If IsNumeric(.YminTxt.Text) And IsNumeric(.YMaxTxt.Text) Then
                    ...
                    Else
                        ChartHandlingMain_Series(AddGraphCollection(.ListBox1.SelectedIndex + 1 - 32))
                        ChartHandlingMain_Range(AddGraphCollection(.ListBox1.SelectedIndex + 1 - 32))
                    End If
                Else
                    MsgBox("Insert Numeric Value!", MsgBoxStyle.Critical, "NaN Error")
                End If
    ...
    Private Sub XscaleTxt_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles XscaleTxt.KeyPress
        If e.KeyChar = ChrW(Keys.Enter) And Me.SeriesLstBox.SelectedIndex <> -1 Then
            ...
            Else
                ChartHandlingMain_Series(AddGraphCollection(Me.ListBox1.SelectedIndex + 1 - 32))
            End If
        End If
    End Sub
    ...
    Private Sub YminTxt_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles YminTxt.KeyPress
        If e.KeyChar = ChrW(Keys.Enter) And Me.SeriesLstBox.SelectedIndex <> -1 Then
            ...
            Else
                ChartHandlingMain_Range(AddGraphCollection(Me.ListBox1.SelectedIndex + 1 - 32))
            End If
        End If
    End Sub
    ...
    Private Sub BtnClr_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnClr.Click
        ...
        With Me
            If .SeriesColor.ShowDialog = Windows.Forms.DialogResult.OK Then
                If .ListBox1.SelectedIndex <= 31 Then
                    SeriesColorChange(GraphCollection(.ListBox1.SelectedIndex + 1))
                Else
                    SeriesColorChange(AddGraphCollection(.ListBox1.SelectedIndex + 1 - 32))
                End If
    ...
    ''**************************************************************************************************************************************************************************
    ''***************************************************** CHART를 Drag & Drop 으로 옮기는 방법 ********************************************************************************
    ''**************************************************************************************************************************************************************************

    'Private Sub Chart00_MouseDown(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles Chart00.MouseDown
    '    '첫번째 : 이벤트 호출
    '    If Me.Chart00.Series.Count = 0 Then Exit Sub
    '    Me.PreviewAnal.Focus()
    '    If e.Button = Windows.Forms.MouseButtons.Middle Then
    '        Me.ListBox1.SelectedIndex = 0
    '        Me.Chart00.DoDragDrop(GraphCollection(Me.ListBox1.SelectedIndex + 1), DragDropEffects.Copy)
    '    End If
    'End Sub

    'Private Sub Chart01_DragEnter(ByVal sender As Object, ByVal e As System.Windows.Forms.DragEventArgs) Handles Chart01.DragEnter
    '    '두번째 : 효과
    '    e.Effect = DragDropEffects.Copy
    'End Sub

    'Private Sub Chart01_DragDrop(ByVal sender As Object, ByVal e As System.Windows.Forms.DragEventArgs) Handles Chart01.DragDrop
    '    '작업을 하는 함수 호출
    '    InjuryChart_DragDropEventEnd(GraphCollection(Me.ListBox1.SelectedIndex + 1), Chart01)
    'End Sub

    'Private Sub InjuryChart_DragDropEventEnd(ByRef MoveChart As Chart, ByRef TChart As Chart)
    '    '세번째 : 작업의 시작
    '    Dim i, j As Integer
    '    Dim tmp As DataPointCollection
    '    Dim Titles As String
    ...
    Private Sub ListBox1_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles ListBox1.KeyDown
        ...
        Try
            ...
            ElseIf e.Modifiers = Keys.Control And e.KeyCode = Keys.C Then
                ...
                Else
                    MovinChrt = AddGraphCollection(SelectedInList - 32)
                End If
    ...
    Private Sub InjuryChart_CopyEventEnd_Primary(ByRef MoveChart As Chart, ByRef TChart As Chart)
        ...
        With Me
            ...
            Next
            TChart.ChartAreas(0).RecalculateAxesScale()
            TChart.ChartAreas(0).AxisY.MajorGrid.Interval = Double.NaN
            TChart.ChartAreas(0).AxisY.MajorTickMark.Interval = Double.NaN
            TChart.ChartAreas(0).AxisY.LabelStyle.Interval = Double.NaN
    ...
    Private Sub ReloadAnalysisDATAToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ReloadAnalysisDATAToolStripMenuItem.Click

        Dim k As Integer = 0
        Dim i As Integer = 0
        Dim NeckGraphNum(8) As Integer

        If GraphBasedCorrelation = True Then
        ...
        Dim ProbarForm As New FrmProgressBar
        With ProbarForm
            .Text = "Now Reloading DATA"
            .Label2.Text = "Reloading Graphs"
            .Label1.Text = "Reloading Cases"
            .ProBar2.Minimum = 0
            .ProBar2.Maximum = 32
            .ProBar1.Minimum = 0
            .ProBar1.Maximum = UBound(Names) + 1
            .Show(Me)
            .Enabled = False
        End With
        ...
        For k = 0 To UBound(Names)
            ...
            If GraphBasedCorrelation = True Then
                'Neck Fx Reading
                Analopen.DataReading(Path(k), Names(k), Graph_Ext(NeckGraphNum(1)), DATA_Pos(NeckGraphNum(1), k, 0), DATA_Pos(NeckGraphNum(1), k, 1), DATA_Tot_Len(NeckGraphNum(1), k, 0), DATA_Tot_Len(NeckGraphNum(1), k, 1), TimeVals, DataVals, ConversionF(NeckGraphNum(1)))
                Neck_Fx = DataVals
                'Neck Fz Reading
                Analopen.DataReading(Path(k), Names(k), Graph_Ext(NeckGraphNum(2)), DATA_Pos(NeckGraphNum(2), k, 0), DATA_Pos(NeckGraphNum(2), k, 1), DATA_Tot_Len(NeckGraphNum(2), k, 0), DATA_Tot_Len(NeckGraphNum(2), k, 1), TimeVals, DataVals, ConversionF(NeckGraphNum(2)))
                Neck_Fz = DataVals
                'Neck My Reading
                Analopen.DataReading(Path(k), Names(k), Graph_Ext(NeckGraphNum(3)), DATA_Pos(NeckGraphNum(3), k, 0), DATA_Pos(NeckGraphNum(3), k, 1), DATA_Tot_Len(NeckGraphNum(3), k, 0), DATA_Tot_Len(NeckGraphNum(3), k, 1), TimeVals, DataVals, ConversionF(NeckGraphNum(3)))
                Neck_My = DataVals
                Neck_Mocy = CalMocyDATA(Neck_Fx, Neck_My, DummyType(k))
                NTE = CalNTEDATA(Neck_Fz, Neck_Mocy, DummyType(k))
                NTF = CalNTFDATA(Neck_Fz, Neck_Mocy, DummyType(k))
            ...
            For i = 0 To 31
                ...
                If GraphBasedCorrelation = True And (DummyType(k) <> 2) Then
                    Select Case i
                        Case NeckGraphNum(4) 'Mocy
                            ReloadSeries(GraphCollection.Item(i + 1), TimeVals, Neck_Mocy, k + 1)
                        Case NeckGraphNum(5) 'NTE
                            ReloadSeries(GraphCollection.Item(i + 1), TimeVals, NTE, k + 1)
                        Case NeckGraphNum(6) 'NTF
                            ReloadSeries(GraphCollection.Item(i + 1), TimeVals, NTF, k + 1)
                        Case NeckGraphNum(7) 'NCE
                            ReloadSeries(GraphCollection.Item(i + 1), TimeVals, NCE, k + 1)
                        Case NeckGraphNum(8) 'NCF
                            ReloadSeries(GraphCollection.Item(i + 1), TimeVals, NCF, k + 1)
                        Case Else
                            Analopen.DataReading(Path(k), Names(k), Graph_Ext(i), DATA_Pos(i, k, 0), DATA_Pos(i, k, 1), DATA_Tot_Len(i, k, 0), DATA_Tot_Len(i, k, 1), TimeVals, DataVals, ConversionF(i))
                    End Select
                Else
                    Analopen.DataReading(Path(k), Names(k), Graph_Ext(i), DATA_Pos(i, k, 0), DATA_Pos(i, k, 1), DATA_Tot_Len(i, k, 0), DATA_Tot_Len(i, k, 1), TimeVals, DataVals, ConversionF(i))
                    ReloadSeries(GraphCollection.Item(i + 1), TimeVals, DataVals, k + 1)
                End If

                ProbarForm.Label2.Text = GraphTitle(i) & " Reloaded"
                ProbarForm.ProBar2.Value = i + 1
                Application.DoEvents()

            Next

            ProbarForm.Label1.Text = "Analysis Case # " & k + 1 & " Reloaded"
            ProbarForm.ProBar1.Value = k + 1
            Application.DoEvents()
        Next

        'MsgBox("Complete Reloading DATA.", MsgBoxStyle.Information, "Complete")
    ...
    Private Sub TempTESTToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TempTESTToolStripMenuItem.Click
        '********* 컨트롤의 영역을 복사하는 방법 *********
